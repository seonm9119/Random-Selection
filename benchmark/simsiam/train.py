...
class SimSiamPairTransform:
    def __init__(self, training_config):
        color_jitter = transforms.ColorJitter(
            training_config["color_jitter_brightness"],
            training_config["color_jitter_contrast"],
            training_config["color_jitter_saturation"],
            training_config["color_jitter_hue"],
        )
        transform_layers = [
            ...
            transforms.RandomApply(
                [GaussianBlur(training_config["gaussian_blur_sigma"])],
                p=training_config["gaussian_blur_probability"],
            ),
...
def resolve_training_config():
    ...
    training_config["scaled_learning_rate"] = (
        ...
        * training_config["batch_size"]
        / training_config["learning_rate_scale_reference_batch_size"]
    )
    return training_config


def train_one_epoch(model, criterion, dataloader, optimizer, learning_rate, scaler, device, training_config, epoch):
    model.train()
    ...
    for step_index, ((first_view_batch, second_view_batch), _) in enumerate(
        ...
        with torch.cuda.amp.autocast(enabled=training_config["amp"]):
            first_prediction, second_prediction, first_projection, second_projection = model(
                first_view_batch,
                second_view_batch,
            )
...
def main():
    ...
    pair_transform = SimSiamPairTransform(training_config)
    ...
    for epoch in range(training_config["training_start_epoch"], last_epoch):
        learning_rate = learning_rate_schedule.get_learning_rate(epoch)
