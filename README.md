...
```bash
./.venv/bin/python scripts/train_benchmarks.py
```
...
```bash
./.venv/bin/python simclr/train.py
```

SimCLR 결과는 `simclr/pretrained/` 바로 아래에 저장되고, 제안 모델 RSCL의 결과는 `rscl/pretrained/` 아래에 저장됩니다.
...
```text
simclr/pretrained/cifar100_batch_1024_best.pt
simclr/pretrained/cifar100_batch_1024_best.json
simclr/pretrained/cifar100_batch_1024_best.log
...
SimCLR 학습 코드는 `simclr/train.py`에 두고, 제안 모델 학습 코드는 `rscl/train.py`에 둡니다. 학습이 끝난 checkpoint를 읽어서 결과를 뽑는 코드는 공용 파일로 분리합니다.
...
./.venv/bin/python linear_eval.py \
  --model simclr \
  --checkpoint simclr/pretrained/cifar100_batch_1024_best.pt \
  --dataset cifar100 \
  --batch-size 512 \
  --epochs 100
```
...
루트 평가 스크립트의 결과 파일은 기본적으로 `output/analysis/` 아래에 JSON으로 저장됩니다. `scripts/` 내부의 실험 보조 스크립트만 `scripts/results/` 아래에 중간 산출물을 저장합니다.
