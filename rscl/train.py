...
def resolve_training_config():
    ...
    if training_config["proj_head_mode"] not in training_config["supported_proj_head_modes"]:
        raise ValueError(f"PROJ_HEAD_MODE must be one of {training_config['supported_proj_head_modes']}.")

    if training_config["optimizer_name"] not in training_config["supported_optimizers"]:
        raise ValueError(f"OPTIMIZER_NAME must be one of {training_config['supported_optimizers']}.")
...
def update_training_step_config(training_config, dataloader):
    if training_config["learning_rate_scaling"] == training_config["learning_rate_scaling_linear"]:
        scaled_learning_rate = training_config["learning_rate"] * training_config["batch_size"]
        scaled_learning_rate /= training_config["learning_rate_scale_reference_batch_size"]
    elif training_config["learning_rate_scaling"] == training_config["learning_rate_scaling_sqrt"]:
        scaled_learning_rate = training_config["learning_rate"] * math.sqrt(training_config["batch_size"])
    else:
        raise ValueError(f"Unknown learning rate scaling {training_config['learning_rate_scaling']}.")

    training_config["scaled_learning_rate"] = scaled_learning_rate
...
def main():
    ...
    pair_transform = RsclPairTransform(training_config)
